...
if(isset($_POST['pedidoExportar'])){
    ...
    class PDF extends tFPDF
    {
        ...
        function Header()
        {
            ...
            $this->SetFont('Exo','',12);
            $this->Cell(70);
            $this->Cell(70,10,'Pedido de Cartao FOP - Pedido '. $this->pedidoExportar,0,1,'C');
    ...
    $query= "SELECT encomendasAnilhas.stam, encomendasAnilhas.clube, MIN(encomendasAnilhas.cartaoFopPago) AS cartoes, socios.nome 
    FROM encomendasAnilhas 
    LEFT JOIN socios
    ON encomendasAnilhas.stam = socios.stam
    WHERE cartaoFopPago = 'sim' AND VagaNum = '$pedidoExportar'  GROUP BY clube,stam";
    ...
    $query = "SELECT COUNT(*) as quantidadeCartaoFop FROM ( SELECT stam, MIN(cartaoFopPago) AS cartoes FROM encomendasAnilhas WHERE cartaoFopPago = 'sim' AND VagaNum = '$pedidoExportar' GROUP BY stam) x";
